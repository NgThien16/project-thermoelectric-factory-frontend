...
import { createWorkOrder } from "../../service/work_order/WorkOrderService";
...
const CreateWorkOrderModal = ({ show, onHide, repairOrder, onCreated }) => {
    ...
    // Reset form ngay trong lúc render khi modal vừa mở - không dùng effect
    if (show && !prevShow) {
        setPrevShow(true);
        setAssignments([]);
        setError("");
    ...
    // Effect chỉ để fetch dữ liệu nhân viên (side-effect thực sự)
    useEffect(() => {
        if (!show) return;
    ...
    const save = async () => {
        ...
        try {
            ...
            toast.success("Tạo phiếu công tác thành công");

            if (onCreated) onCreated(result);
            onHide();

            // Điều hướng sang trang danh sách Phiếu công tác và tự mở modal detail của phiếu mới tạo
            navigate("/work-orders", { state: { openDetailId: result.id } });
        } catch (e) {
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg">
            <Modal.Header closeButton>
                <Modal.Title>Tạo phiếu công tác</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <p><b>{repairOrder?.title}</b></p>
